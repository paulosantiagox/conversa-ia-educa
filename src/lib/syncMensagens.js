...
async function buscarLote(modo, offset = 0) {
  ...
  let query = supabase
    ...
    .select('id, datacrazy_id, contato_nome, contato_numero, consultora, total_mensagens, ultima_mensagem_at, mensagens_sync_at')
    .not('datacrazy_id', 'is', null)
    .eq('precisa_sync_mensagens', true)   // coluna CALCULADA na view ci_conversas (mensagens_sync_at IS NULL OR ultima_mensagem_at > mensagens_sync_at) — auto-mantida, não escrever
    .order('ultima_mensagem_at', { ascending: false })
